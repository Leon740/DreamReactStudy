...
function sanitizeStringFn(string: string): string {
  return string.trim().toLowerCase();
}

// (memo) in order to not to rerender the (CarSearchResults) on (inputValueSt) change
interface CarSearchResultsPropsI {
  data: {
    name: string;
  }[];
  name: string;
}
const CarSearchResults = memo(({ data, name }: CarSearchResultsPropsI) => {
...
function CarSearch(): JSX.Element {
  ...
  const inputOnChangeFn = (value: string): void => {
